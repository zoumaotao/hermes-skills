...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None):
    ...
    # --- Feishu: native media attachment support ---
    if platform == Platform.FEISHU and media_files:
        return await _send_feishu(pconfig, chat_id, " ".join(chunks) if chunks else "", media_files=media_files, thread_id=thread_id)

    # --- Non-media platforms ---
    media_support_platforms = frozenset({"telegram", "discord", "matrix", "weixin", "signal", "yuanbao", "feishu"})
    if media_files and not message.strip():
        if platform.value not in media_support_platforms:
            return {
                "error": (
    ...
    warning = None
    if media_files and platform.value not in media_support_platforms:
        warning = (
